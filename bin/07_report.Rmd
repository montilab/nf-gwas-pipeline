...
for(i in 1:22){
  ...
  try(snp.n.r <- c(snp.n.r, dim(result)[1]))
  result <- NULL
}
...
snp.n.f <- c()
for(i in 1:22){
  try(result <- fread(paste0(outdir,"/Summary_Plot/merge_by_chr/chr_",i,"_caf_annotated.csv")))
  try(snp.n.f <- c(snp.n.f, dim(result)[1]))
  result <- NULL
}
